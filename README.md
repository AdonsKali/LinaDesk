...
| 🖼️ **Визуальный анализ** | Поддержка обработки изображений
| 🔊 **Аудио анализ** | Поддержка аудио файлов
...
| **/*Промпт-действие*/** | **/*Последствие*/** |
|--------|----------|
|**"запомни", "учти", имей "ввиду" ...** | ```Лина попытается запомнить последний полный вопрос = ответ``` | 
|**Перетащите файл на чиби**| ```Лина учтет ваши прикрепленные файлы для последующего ответа (можно прикрепить до 3 файлов за раз)``` |
|**Двойной клик по чиби**| ```Появляется чат и реакция Лины (wake up)```|
...
> **Приложение использует около 2 ГБ оперативной памяти и до 2 ГБ видеопамяти в зависимости от выбранной модели ИИ.**
